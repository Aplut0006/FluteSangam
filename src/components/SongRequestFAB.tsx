...
import { Music, X, Mic, Film, Sparkles, CheckCircle2, ShieldAlert, Send, FileText, Info, KeyRound } from 'lucide-react';
...
export const SongRequestModal: React.FC<SongRequestModalProps> = ({ isOpen, onClose, currentUser, onOpenAuth }) => {
          ...
          {status === 'success' ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="Both Sargam & Western">Both Sargam & Western</option>
                  </select>
                </div>
              </div>

              {/* Notes / Specific Timecode or Instructions */}
              <div>
                <label className="block text-xs font-bold text-gray-700 mb-1.5 flex items-center justify-between">
                  <span className="flex items-center gap-1.5">
                    <FileText className="w-3.5 h-3.5 text-amber-600" />
                    <span>Specific Part or Notes (Optional)</span>
                  </span>
                </label>
                <textarea
                  rows={2}
                  placeholder="e.g. Please provide notes specifically for the intro flute piece or chorus line."
                  className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 focus:bg-white focus:border-amber-600 focus:ring-2 focus:ring-amber-500/20 rounded-xl text-xs text-gray-800 placeholder-gray-400 font-medium outline-none resize-none"
              ...
              <button
                ...
                className="w-full py-3 bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-500 hover:to-amber-600 text-white font-bold text-sm rounded-xl shadow-md hover:shadow-lg transition flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
              >
                <Send className="w-4 h-4" />
                <span>{status === 'submitting' ? 'Submitting Request...' : 'Submit Notation Request'}</span>
              </button>
            </form>
          )}
        </div>
...
export const SongRequestFAB: React.FC<{ isHidden?: boolean; currentUser?: any; onOpenAuth?: () => void }> = () => {
  // FAB floating button is removed per user request
  return null;
};
