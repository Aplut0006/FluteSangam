...
interface HappyBirthdayNotationViewProps {
  onViewChange?: (view: AppView) => void;
}

export const HappyBirthdayNotationView: React.FC<HappyBirthdayNotationViewProps> = ({ 
  onViewChange 
}) => {
  const song = PUBLISHED_SONG_NOTATIONS.find(s => s.slug === 'happy-birthday-flute-notes') || PUBLISHED_SONG_NOTATIONS[0];
      ...
      <div className="max-w-6xl mx-auto px-1 sm:px-6 space-y-6 sm:space-y-10">
        ...
        <header className="bg-gradient-to-br from-amber-700 via-amber-800 to-bamboo-900 rounded-2xl sm:rounded-3xl p-4 sm:p-10 text-white shadow-xl relative overflow-hidden space-y-3 sm:space-y-4">
          ...
          <h1 className="text-xl sm:text-4xl font-extrabold font-display tracking-tight text-white leading-tight">
            Happy Birthday Flute Notes – Easy Sargam &amp; Western Notes
          </h1>
        ...
        <section aria-label="Introduction" className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-8 border border-amber-200 shadow-sm space-y-3 sm:space-y-4">
          <p className="text-xs sm:text-base text-gray-700 leading-relaxed font-sans">
            Learn to play Happy Birthday on flute or bansuri with beginner-friendly Sargam and Western notes. Follow the lyric-aligned phrases, octave markings, breathing guidance and practical tips to practise the complete melody.
          </p>
        </section>
        ...
        <section aria-label="Quick information" className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-8 border border-amber-200 shadow-sm space-y-3 sm:space-y-4">
          <h2 className="text-lg sm:text-2xl font-bold font-display text-bamboo-950 flex items-center gap-2">
            <Target className="w-4 h-4 sm:w-5 sm:h-5 text-amber-600 shrink-0" />
            Quick Information
          </h2>
        ...
        {/* 6. Song Notation */}
        <section aria-label="Song notation" className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-8 border border-amber-200 shadow-sm space-y-4 sm:space-y-6">
          ...
          <p className="text-xs sm:text-sm text-gray-700 leading-relaxed font-sans">
            Sing along or speak the syllables as you play each phrase. Each word is paired directly with its corresponding flute swar below:
          </p>
          ...
          <div className="space-y-3 sm:space-y-6">
            {song.phrases.map((phrase) => (
              <div 
                ...
                className="bg-amber-50/50 rounded-xl sm:rounded-2xl p-3 sm:p-6 border border-amber-200/90 space-y-2.5 sm:space-y-4"
              >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 sm:gap-2.5">
                  {phrase.units.map((unit, uIdx) => (
                    <div 
                      key={uIdx} 
                      className="bg-white rounded-lg sm:rounded-xl p-2 sm:p-3.5 border border-amber-200/80 text-center space-y-1 sm:space-y-2 shadow-2xs hover:border-amber-400 transition flex flex-col justify-center min-h-[56px] sm:min-h-[64px]"
                    >
                      <div className="text-[10px] sm:text-xs font-bold text-amber-900/90 bg-amber-50/90 py-0.5 sm:py-1 px-1.5 sm:px-2 rounded-md border border-amber-100 truncate">
                        {unit.lyric}
                      </div>
                      <div className="font-mono font-black text-sm sm:text-2xl text-bamboo-950 tracking-tight sm:tracking-wider break-words">
                        {unit.sargam}
                      </div>
                    </div>
                  ))}
                ...
                <div className="pt-1 flex flex-col sm:flex-row sm:items-center justify-between gap-1 sm:gap-2 text-[11px] sm:text-xs text-gray-600 bg-white/70 p-2 sm:p-2.5 rounded-lg sm:rounded-xl border border-amber-200/60 font-mono">
                  <span className="text-gray-500 font-sans font-semibold shrink-0">Notation line:</span>
        ...
        <section aria-label="Western notes" className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-8 border border-amber-200 shadow-sm space-y-4 sm:space-y-6">
          ...
          <p className="text-xs sm:text-sm text-gray-700 leading-relaxed font-sans">
            If you are playing on a C flute or keyboard, read the Western notes below where Middle C is used as Sa:
          </p>
          ...
          <div className="space-y-3 sm:space-y-6">
            {song.phrases.map((phrase) => (
              <div 
                key={phrase.phraseNumber}
                className="bg-slate-50 rounded-xl sm:rounded-2xl p-3 sm:p-6 border border-slate-200 space-y-2.5 sm:space-y-4"
              >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 sm:gap-2.5">
                  {phrase.units.map((unit, uIdx) => (
                    <div 
                      key={uIdx} 
                      className="bg-white rounded-lg sm:rounded-xl p-2 sm:p-3.5 border border-slate-200 text-center space-y-1 sm:space-y-2 shadow-2xs flex flex-col justify-center min-h-[56px] sm:min-h-[64px]"
                    >
                      <div className="text-[10px] sm:text-xs font-bold text-slate-600 bg-slate-100 py-0.5 sm:py-1 px-1.5 sm:px-2 rounded-md truncate">
                        {unit.lyric}
                      </div>
                      <div className="font-mono font-black text-sm sm:text-2xl text-slate-900 tracking-tight sm:tracking-wider break-words">
                        {unit.western}
                      </div>
                    </div>
                  ))}
        ...
        {/* 9. Step-by-Step Practice Routine */}
        <section aria-label="Step-by-step practice routine" className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-8 border border-amber-200 shadow-sm space-y-4 sm:space-y-6">
        ...
        <section aria-label="Common mistakes and fixes" className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-8 border border-amber-200 shadow-sm space-y-4 sm:space-y-6">
          <div className="flex items-center gap-2 border-b border-amber-100 pb-2.5 sm:pb-3">
...
export default HappyBirthdayNotationView;
