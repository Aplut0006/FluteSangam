export const VIEW_URLS: Record<string, string> = {
  'community': '/',
  'chats': '/chats',
  'post-detail': '/post',
  'user-profile': '/profile',
  'learn_intro': '/learn/intro',
  'learn_choose_flute': '/learn/choose-flute',
  'learn_tuner': '/tuner',
  'learn_basics': '/learn/basics',
  'learn_fingering_chart': '/learn/fingering-chart',
  'learn_alankaras': '/learn/alankaras',
  'learn_daily_practice': '/learn/daily-practice-guide',
  'learn_scales_octaves': '/learn/flute-scales-octaves',
  'learn_common_mistakes': '/learn/common-flute-mistakes',
  'alankar_generator': '/alankar-generator',
  'learn_raagas': '/learn/raagas',
  'raga_bhoopali': '/learn/raga-bhoopali',
  'raga_durga': '/learn/raga-durga',
  'raga_yaman': '/learn/raga-yaman',
  'raga_hamsadhwani': '/learn/raga-hamsadhwani',
  'raga_bilawal': '/learn/raga-bilawal',
  'raga_desh': '/learn/raga-desh',
  'raga_kafi': '/learn/raga-kafi',
  'raga_bageshree': '/learn/raga-bageshree',
  'raga_bhimpalasi': '/learn/raga-bhimpalasi',
  'raga_brindavani_sarang': '/learn/raga-brindavani-sarang',
  'raga_khamaj': '/learn/raga-khamaj',
  'raga_bhairav': '/learn/raga-bhairav',
  'raga_bihag': '/learn/raga-bihag',
  'raga_malkauns': '/learn/raga-malkauns',
  'raga_marwa': '/learn/raga-marwa',
  'raga_jog': '/learn/raga-jog',
  'raga_todi': '/learn/raga-todi',
  'raga_multani': '/learn/raga-multani',
  'raga_pahadi': '/learn/raga-pahadi',
  'raga_miyan_ki_malhar': '/learn/raga-miyan-ki-malhar',
  'raga_tilang': '/learn/raga-tilang',
  'raga_shivranjani': '/learn/raga-shivranjani',
  'raga_jaunpuri': '/learn/raga-jaunpuri',
  'community_members': '/members',
  'about_us': '/about',
  'founder': '/founder',
  'contact_us': '/contact',
  'learn_dashboard': '/learn',
  'notation_requests': '/notations',
  'notation_happy_birthday': '/notations/happy-birthday-flute-notes',
  'notation_jingle_bells': '/notations/jingle-bells-flute-notes',
  'notation_titanic': '/notations/my-heart-will-go-on-flute-notes',
  'notation_tum_hi_ho': '/notations/tum-hi-ho-flute-notes',
  'notation_kal_ho_naa_ho': '/notations/kal-ho-naa-ho-flute-notes',
  'notation_achyutam_keshavam': '/notations/achyutam-keshavam-flute-notes',
  'notation_radha_krishna': '/notations/radha-krishna-flute-notes',
  'notation_jana_gana_mana': '/notations/jana-gana-mana-flute-notes',
  'privacy_policy': '/privacy-policy',
  'terms_of_service': '/terms-of-service',
  'flute_faq': '/faq',
  'note_key_converter': '/tools/flute-note-key-converter',
  'find_song_scale': '/learn/how-to-find-scale-of-a-song-on-flute',
  'how_to_read_bansuri_notation': '/learn/how-to-read-bansuri-notation',
  'not_found': '/404',
};
